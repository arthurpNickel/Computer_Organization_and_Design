...
main:
	...
	; Pega o endereço de A para usar como base
	; r0 = A
	sub r0, r0
    addi 7
	addi 7
...
soma_vetor:
    ...
    ; r1(aux) = tam - i
	sub r0, r0
	addi 7		; QUESTION: 40 ou 10? Vai ter que ser esse número constante mesmo? PQ MUDOU PARA 0xfa??????????????
	addi 3
	sub r0, r3			; r0 = tam - i
    sub r1, r1          
	...
	; r0 = B[i]
	; r0 = A[i] + 10 + número de instruções adicionadas = B[i]
	
	; r0 += 10 + n
	addi 7			
	addi 3 ; + n (se add mais um tirando essa)

	ld r0, r0		; r0 = B[i]		; QUESTION: Pode isso arnaldo??

	; QUESTION: Sobrou um registrador aqui! Ent será que dá para fazer de outro jeito?
	add r1, r0		; r1 = A[i] + B[i]

	; R[i] = r2 (A[i] + B[i])

	; r0 = i + &R[0] = B[i] + 10 + número de instruções adicionadas
    addi 7
